%% Assignment 7, Problem 4 b)
%  MPC and state feedback, infinite horizon
...
I_Nm1 = eye(N-1);
Qt = diag([4 4]);
Q = kron(I_Nm1, Qt); % 1 block matrix less to "make room for" P
Rt = 1;
R = kron(I_N, Rt);
[~,P,~] = dlqr(A,B,Qt/2,Rt/2,[]); % Find the Riccati matrix P 
G = blkdiag(Q, 2*P, R); % Note that we have to use 2P here!
